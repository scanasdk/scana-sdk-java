...
public final class SM3Digest {
    ...
    private int doFinal(byte[] out, int outOff) {
        ...
        intToBigEndian(this.V[0], out, outOff + 0);
        intToBigEndian(this.V[1], out, outOff + 4);
        intToBigEndian(this.V[2], out, outOff + 8);
        intToBigEndian(this.V[3], out, outOff + 12);
        intToBigEndian(this.V[4], out, outOff + 16);
        intToBigEndian(this.V[5], out, outOff + 20);
        intToBigEndian(this.V[6], out, outOff + 24);
        intToBigEndian(this.V[7], out, outOff + 28);
     ...
     * 
     * 
     * FFj(X;Y;Z) = X XOR Y XOR Z when 0 < = j < = 15 = (X AND Y) OR (X AND Z) OR (Y AND Z) when 16 < = j < = 63
     * 
     * GGj(X;Y;Z) = X XOR Y XOR Z when 0 < = j < = 15 = (X AND Y) OR (NOT X AND Z) when 16 < = j < = 63
     * 
     ...
     * 
     * 
     * P0(X) = X XOR (X <<< 9) XOR (X <<< 17) ## ROLL, not SHIFT P1(X) = X XOR (X <<< 15) XOR (X <<< 23) ## ROLL, not
     * SHIFT
     * 
     ...
     * 
     * ROLL 9 : ((x << 9) | (x >>> (32-9)))) ROLL 17 : ((x << 17) | (x >>> (32-17))) ROLL 15 : ((x << 15) | (x >>>
     * (32-15))) ROLL 23 : ((x << 23) | (x >>> (32-23)))
     * 
     */
    ...
    private void processBlock() {
        System.arraycopy(inwords, 0, W, 0, 16);
